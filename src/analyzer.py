...
Витягує всі поля для шаблонної таблиці: тип звернення, інфо про менеджера,
8 бінарних оцінок по скрипту, тип робіт з переліку "Топ-100", результат,
оцінка 1-10, коментар, проблемність.
"""
...
USER_TEMPLATE = """ПЕРЕЛІК ДОПУСТИМИХ ТИПІВ РОБІТ ("Топ-100 робіт СТО"):
{work_types}

ОЦІНЮВАНІ КРИТЕРІЇ (кожен 0 або 1):
1. greeting — привітався, представився (назвав себе/компанію)
2. asked_body — дізнався кузов автомобіля (модель/тип)
...
7. farewell — попрощався (подякував / "до побачення" / "гарного дня")
8. followed_top100_instructions — дотримувався скрипта/інструкцій для свого типу робіт

is_problematic = true коли:
- менеджер грубий, нецензурна лексика, мат
...
ЗАПЧАСТИНИ: одне з {parts_options} (порожньо якщо не обговорювалось)
...
{{
  ...
  "manager": "<імʼя менеджера або порожньо>",
  "greeting": 0,
  "asked_body": 0,
  "asked_year": 0,
  "asked_mileage": 0,
